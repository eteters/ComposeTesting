...
sealed class Screen(val id: ScreenName) {
    object My_Interviews : Screen(ScreenName.HOME)
    data class Interview_Detail(val meetingId: String) : Screen(ScreenName.DETAIL)
}
...
private fun Screen.toBundle(): Bundle {
    return bundleOf(SIS_NAME to id.name).also {
        ...
        if (this is Screen.Interview_Detail) {
            it.putString(SIS_POST, meetingId)
        }
    }
}
...
private fun Bundle.toScreen(): Screen {
    ...
    return when (screenName) {
        ScreenName.HOME -> Screen.My_Interviews
        ScreenName.DETAIL -> {
            val postId = getStringOrThrow(SIS_POST)
            Screen.Interview_Detail(postId)
        }
    }
}
...
class NavigationViewModel(private val savedStateHandle: SavedStateHandle) : ViewModel() {
    ...
    @MainThread
    fun onBack(): Boolean {
        val wasHandled = currentScreen != Screen.My_Interviews
        currentScreen = Screen.My_Interviews
        return wasHandled
    }
